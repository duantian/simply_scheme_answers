;9.8 Suppose we're writing a program to play hangman. In this game one player has to guess a secret word chosen by
...
(define (hang sent guesses)
  (accumulate word (every (lambda (x) (if (member? x guesses) x '-)) sent)))
